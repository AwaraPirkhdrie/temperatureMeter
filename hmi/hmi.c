...
uint8_t input_int(char *p_msg, uint16_t *p_int_nr)
{
	...
	char numbers[max_length + 1];		// UPPGIFT: Varför måste man dimensionera arrayen för ett extra tecken? \0 behövs som den sista
	uint8_t key;
	...
	while (1) {
		...
		if (key == '#') {					// confirmation of the number?
			...
			if (length > 0) {
				...
				lcd_write(DATA,' '); // REMOVE ONE CHARACTER
				lcd_write(INSTRUCTION, 0x10);
				length--;
			}
